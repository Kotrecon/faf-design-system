...
│   ├── contrast.test.ts              # Unit-тесты утилиты контраста (Vitest)
...
- **Контекст:** `:host-context()` разрешён **только** для контекстных переопределений токенов (маппинг), а не для дублирования state-логики.
- **Слоты:** `::slotted()` используется только для простого оформления projected content, без сложных hover/focus сценариев (для этого у дочерних компонентов есть свой Shadow DOM).
...
2. **FafToast (1080):** Появляется поверх всех элементов. Проверяет контрастность текста через утилиту `@faf/contrast`.
3. **FafDropdown (1000):** Поддерживает навигацию стрелками, закрывается по клику вне области и по `Escape`. Дочерние `FafDropdownItem` имеют собственный Shadow DOM для корректной стилизации `:hover`.
4. **FafTooltip (1070):** Работает при `:hover` и `:focus` (WCAG), использует `aria-describedby`, предотвращает залипание при клике мышью.

---

## ⚙️ Генерация CSS и Тестирование
