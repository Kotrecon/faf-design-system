...
│   │   ├── faf.dropdown.css          # Стили дропдауна
│   │   ├── faf.modal.ts              # 🔥 Модалка (1040/1050 + Focus Trap)
│   │   ├── faf.modal.css             # Стили модалки
│   │   ├── faf.toast.ts              # 🔥 Тост (1080 + Активная проверка контраста)
│   │   ├── faf.toast.css             # Стили тоста
│   │   ├── faf.tooltip.ts            # 🔥 Тултип (1070 + Hover/Focus)
│   │   └── faf.tooltip.css           # Стили тултипа
│   ├── utils/
│   │   ├── focus.ts                  # Хелпер для Focus Trap (заглушка/база)
...
1. Используйте `z-index` только в связке с осознанным позиционированием и токенами `--faf-zindex-*`.
...
| :---------------------------- | :------- | :------------------------------ | :--------------------------- |
| `--faf-zindex-base`           | `0`      | Базовый слой                    | Основной контент страницы    |
| `--faf-zindex-dropdown`       | `1000`   | Встроенные всплывающие элементы | Меню, селекторы              |
| `--faf-zindex-sticky`         | `1020`   | Закреплённые элементы           | Заголовки таблиц             |
| `--faf-zindex-fixed`          | `1030`   | Фиксированные элементы          | Глобальный навбар            |
| `--faf-zindex-modal-backdrop` | `1040`   | Затемнение модального окна      | Backdrop модалки             |
| `--faf-zindex-modal`          | `1050`   | Модальные окна                  | Диалоги                      |
| `--faf-zindex-popover`        | `1060`   | Поповеры                        | Всплывающие панели           |
| `--faf-zindex-tooltip`        | `1070`   | Всплывающие подсказки           | Тултипы при наведении/фокусе |
| `--faf-zindex-toast`          | `1080`   | Глобальные уведомления          | Тосты, системные алерты      |
